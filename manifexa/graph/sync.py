...
def build_graph(vault, cache, engine):
    alias: dict[str, str] = {}
    known: set[str] = set()
    curated_edges: list[tuple[str, str, str]] = []

    # 1. Curated entities are the source of truth; remember their openalex alias
    #    and the hand-drawn [[wikilink]] relationships they carry.
    for entity in vault.list():
        ...
        for rel, target in entity.relations:
            curated_edges.append((entity.id, target, rel))
    ...
    # 3. Edges — curated (hand-drawn) first, then cached candidate edges — with
    #    endpoints remapped through the alias.
    for src, target, rel in curated_edges:
        dst = alias.get(target, target)
        if src in known and dst in known:
            engine.add_edge(src, dst, rel)
